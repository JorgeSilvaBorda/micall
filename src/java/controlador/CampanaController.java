...
public class CampanaController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        switch (entrada.getString("tipo")) {
            ...
            case "select-campanas-new":
                ...
                break;
            case "select-campanas-vta":
                out.print(selectCampanaVenta(request));
                break;
    ...
    public JSONObject selectCampanaVenta(HttpServletRequest request) {
        System.out.println("");
        JSONObject salida = new JSONObject();
        JSONArray campanas = new JSONArray();
        String query = "CALL SP_SELECT_CAMPANAS_IDEMPRESA_SOLO_VENTA(" + Integer.parseInt(request.getSession().getAttribute("idempresa").toString()) + ")";
        Conexion c = new Conexion();
        c.abrir();
        ResultSet rs = c.ejecutarQuery(query);
        String options = modelo.Util.armarSelect(rs, "0", "Seleccione", "IDCAMPANA", "TEXTO");
