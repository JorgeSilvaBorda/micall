...
DROP PROCEDURE IF EXISTS `SP_DEL_VENTA`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_DEL_VENTA` (IN `_IDVENTA` INT)  BEGIN
	DELETE FROM
		VENTA
	WHERE
		IDVENTA = _IDVENTA;
...
DROP PROCEDURE IF EXISTS `SP_DEL_VENTAS_CAMPANA_CLIENTE`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_DEL_VENTAS_CAMPANA_CLIENTE` (IN `_IDCAMPANA` INT, IN `_RUTCLIENTE` INT)  BEGIN
	DELETE FROM
		VENTA
	WHERE
...
CREATE DEFINER=`root`@`%` PROCEDURE `SP_DETALLE_SIMULACIONES_EMPRESA` (IN `_IDEMPRESA` INT, IN `_FECHAINI` DATE, IN `_FECHAFIN` DATE)  BEGIN
...
CREATE DEFINER=`root`@`%` PROCEDURE `SP_DETALLE_VENTAS_EMPRESA` (IN `_IDEMPRESA` INT, IN `_FECHAINI` DATE, IN `_FECHAFIN` DATE)  BEGIN
...
FROM
    ...
    ON B.IDPRODUCTO = C.IDPRODUCTO INNER JOIN EMPRESA D
    ON C.IDEMPRESA = D.IDEMPRESA LEFT JOIN SIMULACIONSUBPRODUCTO E
    ON E.IDSIMULACION = A.IDSIMULACION INNER JOIN VENTA F
    ON A.IDSIMULACION = F.IDSIMULACION
WHERE
	DATE(F.FECHAVENTA) BETWEEN _FECHAINI AND _FECHAFIN
    -- AND D.RUTEMPRESA = (SELECT RUTEMPRESA FROM EMPRESA A INNER JOIN USUARIO B ON A.IDEMPRESA = B.IDEMPRESA WHERE RUTUSUARIO = _RUTUSUARIO)
    AND D.IDEMPRESA = _IDEMPRESA
GROUP BY
	A.IDSIMULACION,
...
CREATE DEFINER=`root`@`%` PROCEDURE `SP_GET_CAMPANA_EMPRESA_RUTCLIETE` (IN `_RUTCLIENTE` INT, IN `_IDEMPRESA` INT)  BEGIN
SELECT
	A.IDRUTERO,
    A.IDCAMPANA,
    A.RUT,
...
FROM
	RUTERO A INNER JOIN (
		...
		GROUP BY
			RUT,
			IDCAMPANA) B 
		ON A.IDRUTERO = B.IDRUTERO
			AND A.IDCAMPANA = B.IDCAMPANA
            AND A.RUT = B.RUT
            AND A.TIPOOPERACION = 'INS' INNER JOIN CAMPANA C
		ON A.IDCAMPANA = C.IDCAMPANA INNER JOIN PRODUCTO D
        ON C.IDPRODUCTO = D.IDPRODUCTO INNER JOIN EMPRESA E 
        ON E.IDEMPRESA = D.IDEMPRESA
WHERE
	A.RUT = _RUTCLIENTE -- 52692512
    AND E.IDEMPRESA = _IDEMPRESA -- 2
    AND MONTH(NOW()) BETWEEN MONTH(C.FECHAINI) AND MONTH(C.FECHAFIN)
    AND YEAR(NOW()) BETWEEN YEAR(C.FECHAINI) AND YEAR(C.FECHAFIN);
END$$

DROP PROCEDURE IF EXISTS `SP_GET_CAMPANA_IDCAMPANA_IDEMPRESA`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GET_CAMPANA_IDCAMPANA_IDEMPRESA` (IN `_IDCAMPANA` INT, IN `_IDEMPRESA` INT, IN `_RUTCLIENTE` INT)  BEGIN
...
DROP PROCEDURE IF EXISTS `SP_GET_SIMULACIONES_RUTVENDEDOR_RUTCLIENTE`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GET_SIMULACIONES_RUTVENDEDOR_RUTCLIENTE` (IN `_RUTVENDEDOR` INT, IN `_RUTCLIENTE` INT)  BEGIN
SELECT DISTINCT
	A.IDSIMULACION,
    A.IDCAMPANA,
    B.CODCAMPANA,
...
SELECT
    ...
    B.DESCSUBPRODUCTO,
    B.PRIMA,
    ((B.PRIMA / 100) * C.MONTO) MONTOSEGURO,
    E.MONTOMETA,
    E.CANTIDADMETA
FROM
...
FROM
    ...
    ON B.IDPRODUCTO = C.IDPRODUCTO INNER JOIN EMPRESA D
    ON C.IDEMPRESA = D.IDEMPRESA LEFT JOIN SIMULACIONSUBPRODUCTO E
    ON A.IDSIMULACION = E.IDSIMULACION INNER JOIN VENTA F
    ON A.IDSIMULACION = F.IDSIMULACION
WHERE
	A.RUTVENDEDOR = _RUTVENDEDOR
    AND DATE(F.FECHAVENTA) BETWEEN _DESDE AND _HASTA
GROUP BY
	A.IDSIMULACION,
    A.IDCAMPANA,
    F.FECHAVENTA,
...
DROP PROCEDURE IF EXISTS `SP_GET_VENTA_API`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GET_VENTA_API` (IN `_RUTCLIENTE` INT, IN `_FECHAVENTA` DATE, IN `_RUTEMPRESA` INT, IN `_CODCAMPANA` VARCHAR(60))  BEGIN
	DECLARE _IDCAMPANA INT;
	SELECT IDCAMPANA INTO _IDCAMPANA FROM CAMPANA WHERE CODCAMPANA = _CODCAMPANA;
SELECT
	A.IDSIMULACION,
    A.IDCAMPANA,
    DATE(A.FECHASIMULACION) FECHASIMULACION,
...
FROM
    ...
    ON C.IDEMPRESA = D.IDEMPRESA LEFT JOIN SIMULACIONSUBPRODUCTO E
    ON A.IDSIMULACION = E.IDSIMULACION LEFT JOIN SUBPRODUCTO F
    ON E.IDSUBPRODUCTO = F.IDSUBPRODUCTO INNER JOIN VENTA G
    ON A.IDSIMULACION = G.IDSIMULACION
WHERE
	A.RUTCLIENTE = _RUTCLIENTE
    -- AND DATE(A.FECHASIMULACION) = _FECHASIMULACION
    AND DATE(G.FECHAVENTA) = _FECHAVENTA
    AND D.RUTEMPRESA = _RUTEMPRESA
...
DROP PROCEDURE IF EXISTS `SP_INS_VENTA`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_INS_VENTA` (IN `_IDSIMULACION` INT, IN `_RUTCLIENTE` INT, IN `_RUTVENDEDOR` INT)  BEGIN
	DECLARE _IDCAMPANA INT;
...
DROP PROCEDURE IF EXISTS `SP_RESUMEN_MES_VENTAS_VENDEDOR`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_RESUMEN_MES_VENTAS_VENDEDOR` (IN `_RUTVENDEDOR` INT)  BEGIN
SELECT
	B.IDCAMPANA,
    B.CODCAMPANA,
    B.NOMCAMPANA,
...
FROM
    ...
    ON A.IDCAMPANA = B.IDCAMPANA INNER JOIN PRODUCTO C
    ON B.IDPRODUCTO = C.IDPRODUCTO INNER JOIN EMPRESA D
    ON C.IDEMPRESA = D.IDEMPRESA INNER JOIN VENTA E
    ON A.IDSIMULACION = E.IDSIMULACION
WHERE
	A.RUTVENDEDOR = _RUTVENDEDOR
    AND (E.FECHAVENTA BETWEEN (LAST_DAY(NOW() - INTERVAL 1 MONTH) + INTERVAL 1 DAY) AND LAST_DAY(NOW())
    OR E.FECHAVENTA IS NULL)
GROUP BY
	B.IDCAMPANA,
    B.CODCAMPANA,
    B.NOMCAMPANA,
...
DROP PROCEDURE IF EXISTS `SP_RESUMEN_VENTAS_EMPRESA_DIA`$$
CREATE DEFINER=`root`@`%` PROCEDURE `SP_RESUMEN_VENTAS_EMPRESA_DIA` (IN `_IDEMPRESA` INT)  BEGIN

-- SET @pIDEMPRESA := 2;
SET @pMESACTUAL := MONTH(NOW());
SET @pANIOACTUAL := YEAR(NOW());
SET @pDIAACTUAL := DAY(NOW());

DROP TABLE IF EXISTS TBL_CAMPANAS;
CREATE TEMPORARY TABLE TBL_CAMPANAS -- TODAS LAS CAMPAÑAS CON SUS PRODUCTOS Y SUBPRODUCTOS PARA LA EMPRESA REVISADA
-- SI HAY PRODUCTOS SIN CAMPAÑA, NO APARECERÁ AQUI
SELECT
	A.IDCAMPANA,
    A.CODCAMPANA,
    A.NOMCAMPANA,
...
FROM
	CAMPANA A INNER JOIN PRODUCTO B
    ON A.IDPRODUCTO = B.IDPRODUCTO LEFT JOIN CAMPANASUBPRODUCTO C
    ON A.IDCAMPANA = C.IDCAMPANA LEFT JOIN SUBPRODUCTO D
    ON C.IDSUBPRODUCTO = D.IDSUBPRODUCTO
WHERE
	B.IDEMPRESA = _IDEMPRESA
    AND D.IDEMPRESA = _IDEMPRESA
    AND @pMESACTUAL BETWEEN MONTH(A.FECHAINI) AND MONTH(A.FECHAFIN)
	AND @pANIOACTUAL BETWEEN YEAR(A.FECHAINI) AND YEAR(A.FECHAFIN)
ORDER BY
	A.CODCAMPANA,
    A.NOMCAMPANA,
    B.CODPRODUCTO,
...
DROP TABLE IF EXISTS TBL_RESUMEN_CAMPANA;
CREATE TEMPORARY TABLE TBL_RESUMEN_CAMPANA
SELECT -- TABLA DE RESUMEN DE VENTAS POR CAMPAÑA
	COUNT(B.IDVENTA) CANT,
    A.IDCAMPANA,
    SUM(A.MONTO) ACUM
FROM
	SIMULACION A INNER JOIN VENTA B
    ON A.IDSIMULACION = B.IDSIMULACION
WHERE
	MONTH(B.FECHAVENTA) = @pMESACTUAL
	AND YEAR(B.FECHAVENTA) = @pANIOACTUAL
    AND DAY(B.FECHAVENTA) = @pDIAACTUAL
GROUP BY
    A.IDCAMPANA
    ;

-- SELECT FINAL -- CRUCE CON RESUMEN VENTAS -----------------------------------------
SELECT
	A.IDCAMPANA,
    A.CODCAMPANA,
    A.NOMCAMPANA,
    ...
    A.CODPRODUCTO,
    A.DESCPRODUCTO,
    CASE WHEN B.CANT IS NULL THEN
		0
	ELSE
		B.CANT
	END AS SIMULACIONESDIA,
    CASE WHEN B.ACUM IS NULL THEN 
		0
	ELSE
		B.ACUM
	END AS ACUMPRODDIA,
	(100 * (IFNULL(B.ACUM, 0))) / A.META PORCACUMDIA,
    A.IDSUBPRODUCTO,
    A.CODSUBPRODUCTO,
    A.DESCSUBPRODUCTO,
    A.MONTOMETA,
    A.CANTIDADMETA,
    A.PRIMA,
    IFNULL(FN_COUNT_SUBPRODUCTOS_VENDIDOS_DIA(A.IDCAMPANA, A.IDSUBPRODUCTO), 0) CANTDIA,
    CASE
		WHEN FN_COUNT_SUBPRODUCTOS_VENDIDOS_DIA(A.IDCAMPANA, A.IDSUBPRODUCTO) IS NULL THEN
			0
		ELSE
			((A.PRIMA / 100) *(CASE WHEN B.ACUM IS NULL THEN 0 ELSE B.ACUM END)) * FN_COUNT_SUBPRODUCTOS_VENDIDOS_DIA(A.IDCAMPANA, A.IDSUBPRODUCTO)
	END AS ACUMDIA,
    ((CASE
		WHEN FN_COUNT_SUBPRODUCTOS_VENDIDOS_DIA(A.IDCAMPANA, A.IDSUBPRODUCTO) IS NULL THEN
			0
		ELSE
			((A.PRIMA / 100) *(CASE WHEN B.ACUM IS NULL THEN 0 ELSE B.ACUM END)) * FN_COUNT_SUBPRODUCTOS_VENDIDOS_DIA(A.IDCAMPANA, A.IDSUBPRODUCTO)
	END) * 100) / A.MONTOMETA PORACUMDIA
FROM
	TBL_CAMPANAS A LEFT JOIN TBL_RESUMEN_CAMPANA B
...
GROUP BY
    ...
    A.CANTIDADMETA,
    A.PRIMA,
    IFNULL(FN_COUNT_SUBPRODUCTOS_VENDIDOS_DIA(A.IDCAMPANA, A.IDSUBPRODUCTO), 0),
    CASE
		WHEN FN_COUNT_SUBPRODUCTOS_VENDIDOS_DIA(A.IDCAMPANA, A.IDSUBPRODUCTO) IS NULL THEN
			0
		ELSE
			((A.PRIMA / 100) *(CASE WHEN B.ACUM IS NULL THEN 0 ELSE B.ACUM END)) * FN_COUNT_SUBPRODUCTOS_VENDIDOS_DIA(A.IDCAMPANA, A.IDSUBPRODUCTO)
	END,
    (100 * (IFNULL(B.ACUM, 0))) / A.META 
;
END$$

DROP PROCEDURE IF EXISTS `SP_RESUMEN_VENTAS_EMPRESA_MES`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_RESUMEN_VENTAS_EMPRESA_MES` (IN `_IDEMPRESA` INT)  BEGIN
...
SELECT
	...
	ELSE
		B.ACUM
	END AS ACUMPROD,
	(100 * (IFNULL(B.ACUM, 0))) / A.META PORCACUM,
    A.IDSUBPRODUCTO,
    A.CODSUBPRODUCTO,
    A.DESCSUBPRODUCTO,
    A.MONTOMETA,
    A.CANTIDADMETA,
    A.PRIMA,
    IFNULL(FN_COUNT_SUBPRODUCTOS_VENDIDOS_MES(A.IDCAMPANA, A.IDSUBPRODUCTO), 0) CANTMES,
    CASE
		WHEN FN_COUNT_SUBPRODUCTOS_VENDIDOS_MES(A.IDCAMPANA, A.IDSUBPRODUCTO) IS NULL THEN
			0
		ELSE
			((A.PRIMA / 100) *(CASE WHEN B.ACUM IS NULL THEN 0 ELSE B.ACUM END)) * FN_COUNT_SUBPRODUCTOS_VENDIDOS_MES(A.IDCAMPANA, A.IDSUBPRODUCTO)
	END AS ACUMMES,
    ((CASE
		WHEN FN_COUNT_SUBPRODUCTOS_VENDIDOS_MES(A.IDCAMPANA, A.IDSUBPRODUCTO) IS NULL THEN
			0
		ELSE
			((A.PRIMA / 100) *(CASE WHEN B.ACUM IS NULL THEN 0 ELSE B.ACUM END)) * FN_COUNT_SUBPRODUCTOS_VENDIDOS_MES(A.IDCAMPANA, A.IDSUBPRODUCTO)
	END) * 100) / A.MONTOMETA PORCACUMMES
FROM
	TBL_CAMPANAS A LEFT JOIN TBL_RESUMEN_CAMPANA B
    ON A.IDCAMPANA = B.IDCAMPANA
GROUP BY
    ...
    A.CANTIDADMETA,
    A.PRIMA,
    IFNULL(FN_COUNT_SUBPRODUCTOS_VENDIDOS_MES(A.IDCAMPANA, A.IDSUBPRODUCTO), 0),
    CASE
		WHEN FN_COUNT_SUBPRODUCTOS_VENDIDOS_MES(A.IDCAMPANA, A.IDSUBPRODUCTO) IS NULL THEN
			0
		ELSE
...
-- Funciones
--
DROP FUNCTION IF EXISTS `FN_COUNT_SUBPRODUCTOS_VENDIDOS_DIA`$$
CREATE DEFINER=`root`@`%` FUNCTION `FN_COUNT_SUBPRODUCTOS_VENDIDOS_DIA` (`_IDCAMPANA` INT, `_IDSUBPRODUCTO` INT) RETURNS INT(11) BEGIN
DECLARE _CANTIDAD INT; -- PARÁMETRO DE SALIDA

DECLARE _DIAACTUAL INT;
DECLARE _MESACTUAL INT;
DECLARE _ANIOACTUAL INT;

SELECT DAY(NOW()) INTO _DIAACTUAL;
SELECT MONTH(NOW()) INTO _MESACTUAL;
SELECT YEAR(NOW()) INTO _ANIOACTUAL;

SELECT
	COUNT(B.IDSUBPRODUCTO)
INTO
	_CANTIDAD
FROM
	SIMULACION A LEFT JOIN SIMULACIONSUBPRODUCTO B
    ON A.IDSIMULACION = B.IDSIMULACION INNER JOIN VENTA C
    ON A.IDSIMULACION = C.IDSIMULACION
WHERE
	A.IDCAMPANA = _IDCAMPANA
    AND B.IDSUBPRODUCTO = _IDSUBPRODUCTO
    AND MONTH(C.FECHAVENTA) = _MESACTUAL
    AND YEAR(C.FECHAVENTA) = _ANIOACTUAL
    AND DAY(C.FECHAVENTA) = _DIAACTUAL
GROUP BY
	A.IDCAMPANA,
    B.IDSUBPRODUCTO;
RETURN _CANTIDAD;
...
DROP FUNCTION IF EXISTS `FN_COUNT_SUBPRODUCTOS_VENDIDOS_MES`$$
CREATE DEFINER=`root`@`localhost` FUNCTION `FN_COUNT_SUBPRODUCTOS_VENDIDOS_MES` (`_IDCAMPANA` INT, `_IDSUBPRODUCTO` INT) RETURNS INT(11) BEGIN
DECLARE _CANTIDAD INT; -- PARÁMETRO DE SALIDA
...
(4, 1, 'Crédito 1 Consolidado', 'CRED_001_CON', '2019-09-01', '2019-09-30', 2000000000);
...
(6, 4, 11948618, '1', 'Owen Booker', 'Bonner Mann', 'M', '1991-12-21', 'Apdo.:900-2789 Cras Av.', 'Empedrado', 'Maule', 283212, 'id.ante.Nunc@lobortisquama.edu', 8250523, 973585349, 247112892, 260823789, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 40641475, '2', 'Hilel Oliver', 'Carpenter Macdonald', 'F', '1987-01-26', 'Apdo.:867-3307 Consectetuer Carretera', 'Providencia', 'RM', 47907, 'ultrices.mauris@mollis.net', 9350291, 346485264, 806508611, 253775172, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 5269251, '2', 'Lucius Newton', 'Fox Romero', 'M', '1999-09-15', '647-2866 Lacinia Avda.', 'Talca', 'VII', 728388, 'Morbi.vehicula@venenatisvel.co.uk', 6190967, 584121763, 433292597, 713189803, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 46606752, '0', 'Darius Mckee', 'Langley Figueroa', 'F', '1989-01-06', 'Apdo.:252-9252 Eget Ctra.', 'Yumbel', 'VII', 530595, 'at.augue.id@laciniaatiaculis.org', 2925106, 917903153, 203549876, 249128546, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 21024502, '2', 'Raphael Stone', 'Blanchard Neal', 'M', '1997-10-05', '2585 Praesent Carretera', 'Ovalle', 'Coquimbo', 487316, 'Quisque@Seddiam.co.uk', 9198866, 295090801, 692903138, 438764178, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 31316968, '5', 'Lance Parker', 'Barnes Valencia', 'F', '1993-05-13', '609-7512 Orci, Carretera', 'Paine', 'Metropolitana de Santiago', 210328, 'elementum.lorem@enimconsequatpurus.com', 4503061, 123889543, 999148155, 154329839, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 12741790, '3', 'Adrian Tyler', 'Shepherd Abbott', 'F', '1986-06-26', '807-3759 Mauris Avenida', 'San Bernardo', 'Metropolitana de Santiago', 913786, 'ac@habitant.com', 4878060, 381953968, 258857499, 223225576, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 7532039, '6', 'Kamal Stafford', 'Hickman Rios', 'F', '1992-01-05', '332-1514 Dignissim Avenida', 'Paiguano', 'IV', 211954, 'ultrices@nectempus.co.uk', 2757253, 826961204, 883300522, 492274100, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 46298247, 'K', 'Kennedy Maynard', 'Ferrell Doyle', 'F', '1986-03-27', '769-9163 Malesuada Av.', 'Talagante', 'Metropolitana de Santiago', 516811, 'lacus@Aliquam.net', 1356880, 510333388, 296607020, 122748151, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 24850673, '3', 'Marshall Britt', 'Merrill Good', 'F', '1988-11-08', '251 Arcu. Ctra.', 'Cerro Navia', 'RM', 767820, 'pede.et.risus@metusInlorem.ca', 2900523, 523635565, 470838571, 656607309, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 19475304, '7', 'Aristotle Heath', 'Bishop Stevens', 'M', '2000-01-02', '860-1326 Sit ', 'Coquimbo', 'IV', 771325, 'sapien.molestie.orci@dolordapibus.com', 1126448, 571350999, 511119881, 255533629, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 36426421, '6', 'Cullen Alvarez', 'Clemons Carrillo', 'M', '1989-11-14', '436 A, Avda.', 'Nancagua', 'VI', 581442, 'gravida@Nullamlobortis.ca', 2292001, 616960171, 208302545, 745982762, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 18074302, '2', 'Aristotle Cooley', 'Nelson Wilson', 'M', '1991-02-25', '2095 Facilisis Avenida', 'San Bernardo', 'RM', 15797, 'est.ac@Nunc.edu', 8340921, 669838441, 928282311, 375265257, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 27814958, '7', 'Nathan Hubbard', 'Cox Yates', 'F', '1987-03-24', '2056 Amet, Avenida', 'Lo Espejo', 'Metropolitana de Santiago', 307925, 'vel.mauris.Integer@turpisIn.co.uk', 3036149, 501968758, 240043421, 315047448, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 12037714, '0', 'Zeph Edwards', 'Bartlett Meyer', 'F', '1991-09-18', '385-4001 Consectetuer Ctra.', 'Cerro Navia', 'RM', 386210, 'sed.orci@volutpatNulla.co.uk', 7326797, 230251943, 925666906, 272045428, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 42851097, '6', 'Dante Wilder', 'Rasmussen Rosa', 'F', '1993-01-10', '3121 Vestibulum Calle', 'San Bernardo', 'Metropolitana de Santiago', 2167, 'Nulla.facilisi.Sed@egestas.edu', 5006776, 354178819, 379036809, 792576956, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 19617135, '5', 'Donovan Cherry', 'Pickett Matthews', 'F', '1991-08-30', '149-334 Amet Ctra.', 'Illapel', 'Coquimbo', 330624, 'ultrices@sed.org', 5597853, 687672498, 603043391, 889049109, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 9530317, 'K', 'Tyrone Blevins', 'Bonner Carter', 'F', '1990-01-23', 'Apdo.:205-5783 Orci. Avenida', 'Pirque', 'Metropolitana de Santiago', 182727, 'convallis.ante@ipsumcursus.edu', 2244853, 347460463, 691173493, 647717423, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 9346659, '4', 'Connor Morris', 'Owens Pollard', 'F', '2000-08-25', '6181 Fermentum Avda.', 'Dalcahue', 'Los Lagos', 69110, 'posuere.cubilia@neque.edu', 5410613, 442002110, 913863680, 118624000, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 50122764, '1', 'Perry Cooke', 'Hopper Nunez', 'M', '2000-05-09', '7632 Varius C/', 'La Reina', 'RM', 178667, 'tincidunt.congue.turpis@viverraMaecenasiaculis.edu', 8573464, 829080892, 551166133, 450616924, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 21577240, '3', 'Conan Bell', 'Schmidt Keith', 'M', '1997-02-26', '4165 Eu Avda.', 'Villa Alegre', 'VII', 497391, 'Quisque@tellus.com', 8852779, 257706685, 317186448, 194422135, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 32478200, '1', 'Sawyer Faulkner', 'Golden Burris', 'M', '1986-10-11', '5084 Lacinia. C/', 'Isla de Maipo', 'Metropolitana de Santiago', 802772, 'gravida@neque.net', 1732503, 549814424, 160690247, 859548397, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 17323637, '9', 'Yuli Watson', 'Kline Norton', 'M', '2000-05-03', '196-6875 Non Carretera', 'Quilicura', 'RM', 825373, 'dolor.Fusce.feugiat@ametconsectetuer.ca', 6237595, 898432075, 346924221, 289513184, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 29781886, '4', 'Hamilton Jackson', 'Serrano Crosby', 'F', '1989-08-06', '745 Sociosqu ', 'Padre Hurtado', 'RM', 899534, 'eros@Duis.edu', 5129734, 488389967, 615971809, 773564880, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 37496235, '3', 'Burton Whitney', 'Hughes Sanders', 'F', '1994-10-28', '2322 Dis ', 'San Felipe', 'V', 184659, 'lobortis.quam.a@egetipsumSuspendisse.org', 1318927, 510084248, 851231608, 758018864, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 32105668, '7', 'Hakeem Hansen', 'Townsend Villarreal', 'F', '1989-04-30', '397-1571 Purus. Ctra.', 'Curarrehue', 'IX', 745518, 'Sed.congue@velitinaliquet.com', 7403903, 537281731, 647055105, 444515976, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 12319901, '4', 'Walker Mckenzie', 'Carrillo Clements', 'F', '1997-09-29', '724-303 Phasellus Avda.', 'Cerrillos', 'RM', 643232, 'ligula.tortor.dictum@famesac.co.uk', 8448373, 777678537, 485014077, 717380763, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 13387419, '4', 'Wang Fulton', 'Larson Gilbert', 'M', '2000-11-27', '533 Purus ', 'Parral', 'VII', 206077, 'sapien@Nullaeuneque.co.uk', 4697347, 133139553, 984402245, 837819058, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 13797589, '0', 'John Tillman', 'Bender Holland', 'M', '1999-05-25', '4656 Pede, Carretera', 'Lo Espejo', 'RM', 712694, 'condimentum.eget.volutpat@Nullam.ca', 4486013, 255714723, 661868222, 672859090, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 41747453, '6', 'Edward Alvarez', 'Lyons Sosa', 'M', '1992-12-03', '771 Mi, Carretera', 'Huechuraba', 'Metropolitana de Santiago', 471594, 'nisl.sem@dictumultriciesligula.edu', 6791205, 113935669, 671281288, 554046172, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 49853940, '8', 'Xavier Schwartz', 'Stout Willis', 'M', '1997-10-08', '984-1918 Nisi C.', 'Colchane', 'I', 898520, 'dictum@ac.ca', 4204270, 858942650, 975800779, 606958818, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 44085353, '6', 'Kennan Chandler', 'Sawyer Lowery', 'F', '1986-10-05', '691-9345 Arcu. Carretera', 'Sierra Gorda', 'Antofagasta', 260450, 'nec.cursus@ornarelectusjusto.co.uk', 8590927, 737817813, 938620621, 474315208, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 41924203, '9', 'Kaseem Hurst', 'Ayers Mayer', 'F', '1990-12-17', '4035 Sed Calle', 'Pelluhue', 'VII', 109293, 'magnis@ridiculusmusProin.org', 8662100, 125306572, 538019535, 639777633, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 26585164, '9', 'Lawrence Vaughan', 'Allen Moore', 'F', '1989-12-18', '622-2763 Urna. C.', 'Las Condes', 'Metropolitana de Santiago', 440443, 'Vivamus.euismod@feugiatnec.com', 5162788, 854772553, 186605551, 538025293, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 12566210, '2', 'Ivan Spence', 'Vance Blackwell', 'F', '1986-04-30', '229-4422 Nibh. Ctra.', 'MaipÃº', 'Metropolitana de Santiago', 182953, 'consequat.dolor@Donecconsectetuermauris.com', 6173326, 377928463, 199801682, 661024137, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 28083441, '6', 'Jerome Vincent', 'Drake Dotson', 'F', '1988-09-20', '2005 Placerat, Av.', 'Mejillones', 'Antofagasta', 260002, 'ipsum.dolor.sit@iaculisquispede.org', 5758586, 665611988, 126375836, 287590738, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 31542065, '2', 'Elijah Garza', 'Atkinson Turner', 'M', '1996-03-29', '9168 Lacinia Avda.', 'Punta Arenas', 'XII', 704919, 'amet.consectetuer.adipiscing@mi.org', 3603436, 656269571, 994339650, 328996152, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 11356819, '4', 'Walker Burch', 'Patterson Walters', 'M', '1992-09-27', '6747 Ullamcorper, Avenida', 'La Cisterna', 'RM', 301828, 'varius@elit.ca', 7959229, 111464560, 756043384, 774194491, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4),
(6, 4, 39025580, '2', 'Brenden Clark', 'Carlson Owens', 'M', '1986-02-26', '533-7814 Eu, Carretera', 'Isla de Maipo', 'RM', 853211, 'natoque.penatibus@neque.co.uk', 6739477, 626220995, 407188041, 571695489, 'RUTERO_1.csv', '2019-09-02 12:44:02', 'INS', 4);
...
INSERT INTO `SIMULACION` (`IDSIMULACION`, `IDCAMPANA`, `FECHASIMULACION`, `RUTVENDEDOR`, `DVVENDEDOR`, `RUTCLIENTE`, `DVCLIENTE`, `MONTO`, `CUOTAS`, `VALORCUOTA`, `TASAINTERES`, `TASAANUAL`, `CAE`, `VENCIMIENTO`, `COSTOTOTAL`, `COMISION`, `IMPUESTO`) VALUES
(1, 1, '2019-09-01 10:36:46', 19678957, 'k', 11948618, '1', 8000000, 36, 250000, '2.90', '34.80', '29.00', '2019-10-15', 9000000, 15500, NULL),
(2, 1, '2019-09-01 10:39:18', 19678957, 'k', 19475304, '7', 1100000, 24, 72000, '2.30', '27.60', '23.00', '2019-10-30', 1728000, 8500, NULL),
(3, 1, '2019-09-01 10:41:34', 19678957, 'k', 27814958, '7', 3000000, 48, 94000, '2.50', '30.00', '25.00', '2019-10-30', 4512000, 23000, NULL),
(4, 1, '2019-09-02 12:18:08', 19678957, 'k', 31542065, '2', 3000000, 36, 99000, '2.50', '30.00', '25.00', '2019-10-31', 3564000, 4000, NULL),
(5, 1, '2019-09-02 12:24:19', 19678957, 'k', 39025580, '2', 6000000, 48, 145000, '2.30', '27.60', '23.00', '2019-10-31', 6960000, 12500, NULL),
(6, 4, '2019-09-02 12:49:47', 19678957, 'k', 11948618, '1', 5500000, 24, 280000, '2.30', '27.60', '23.00', '2019-10-31', 6720000, 4500, NULL),
(7, 1, '2019-09-04 12:30:52', 19678957, 'k', 28083441, '6', 8000000, 24, 450000, '2.30', '27.60', '23.00', '2019-10-31', 10800000, 12500, NULL),
(8, 1, '2019-09-04 12:39:36', 19678957, 'k', 44085353, '6', 7500000, 36, 260000, '2.30', '27.60', '23.00', '2019-10-30', 9360000, 11200, NULL),
(9, 1, '2019-09-04 12:49:42', 19678957, 'k', 17323637, '9', 5500000, 36, 152778, '1.80', '21.60', '18.00', '2019-10-18', 7020000, 9500, NULL),
(10, 1, '2019-09-04 13:02:19', 19678957, 'k', 29781886, '4', 3500000, 24, 195000, '1.90', '22.80', '19.00', '2019-10-25', 4680000, 5600, NULL),
(11, 1, '2019-09-04 13:05:36', 19678957, 'k', 12319901, '4', 8500000, 60, 195000, '3.20', '38.40', '32.00', '2019-09-30', 11700000, 14500, NULL),
(12, 1, '2019-09-07 11:12:56', 16355662, '6', 11948618, '1', 7000000, 10, 750000, '1.20', '14.40', '25.00', '2019-11-16', 7500000, 0, NULL),
(13, 4, '2019-09-07 12:40:30', 16355662, '6', 11948618, '1', 6000000, 10, 650000, '1.20', '14.40', '22.00', '2019-11-30', 7000000, 10000, NULL),
(14, 4, '2019-09-07 15:03:43', 16355662, '6', 11948618, '1', 8573464, 10, 857347, '1.20', '14.40', '23.00', '2020-01-19', 9000000, 20000, NULL),
(15, 1, '2019-09-08 03:08:25', 19678957, 'k', 11948618, '1', 2000000, 24, 100000, '1.50', '18.00', '27.00', '2019-09-30', 2400000, 1000, NULL),
(16, 4, '2019-09-08 13:27:03', 19678957, 'k', 40641475, '2', 3500000, 12, 350000, '1.00', '12.00', '23.00', '2019-10-12', 4200000, 2500, NULL),
(17, 4, '2019-09-08 13:30:33', 19678957, 'k', 40641475, '2', 3500000, 24, 200000, '1.00', '12.00', '19.50', '2019-09-30', 4800000, 4000, NULL),
(18, 4, '2019-09-08 13:37:39', 19678957, 'k', 31542065, '2', 5000000, 12, 600000, '1.00', '12.00', '24.00', '2019-09-30', 7200000, 4000, NULL),
(19, 4, '2019-09-08 13:38:41', 19678957, 'k', 31542065, '2', 5000000, 24, 300000, '1.20', '14.40', '25.00', '2019-09-30', 7200000, 4000, NULL),
(20, 4, '2019-09-08 13:50:41', 18667642, '4', 39025580, '2', 2000000, 20, 120000, '2.30', '27.60', '23.00', '2019-10-31', 2400000, 12500, NULL),
(21, 4, '2019-09-08 13:53:53', 18667642, '4', 39025580, '2', 4000000, 20, 220000, '2.30', '27.60', '23.00', '2019-10-30', 4800000, 12500, NULL),
(22, 4, '2019-09-08 13:54:31', 19838453, '4', 42851097, '6', 1000000, 24, 50000, '1.00', '12.00', '21.00', '2019-09-30', 1200000, 2000, NULL),
(23, 4, '2019-09-08 13:55:15', 19838453, '4', 42851097, '6', 1000000, 48, 25000, '1.00', '12.00', '19.00', '2019-09-30', 1200000, 4000, NULL),
(24, 4, '2019-09-08 13:58:18', 18667642, '4', 39025580, '2', 400000, 24, 190000, '2.30', '27.60', '23.00', '2019-10-30', 4560000, 12500, NULL),
(25, 1, '2019-09-08 14:10:33', 18667642, '4', 11356819, '4', 8000000, 48, 220000, '2.30', '27.60', '23.00', '2019-09-30', 10560000, 14500, NULL),
(26, 4, '2019-09-08 14:12:43', 18667642, '4', 11356819, '4', 8000000, 36, 289000, '2.30', '27.60', '23.00', '2019-09-23', 10404000, 10500, NULL),
(27, 4, '2019-09-08 14:12:45', 19678957, 'k', 11948618, '1', 6000000, 10, 650000, '1.20', '14.40', '25.00', '2019-12-28', 6500000, 45000, NULL),
(28, 4, '2019-09-08 14:16:26', 18667642, '4', 31542065, '2', 1500000, 24, 84000, '2.30', '27.60', '23.00', '2019-09-30', 2016000, 4500, NULL),
(29, 4, '2019-09-08 14:18:25', 18667642, '4', 31542065, '2', 1500000, 48, 45000, '2.30', '27.60', '234.00', '2019-09-30', 2160000, 8500, NULL),
(30, 4, '2019-09-08 14:22:36', 19838453, '4', 32105668, '7', 1000000, 24, 50000, '1.00', '12.00', '23.00', '2019-09-30', 1200000, 4000, NULL),
(31, 1, '2019-09-08 14:47:53', 18667642, '4', 28083441, '6', 6000000, 36, 190000, '2.30', '27.60', '23.00', '2019-09-30', 6840000, 12000, NULL),
(32, 1, '2019-09-08 14:50:48', 18667642, '4', 28083441, '6', 6000000, 48, 145000, '2.30', '27.60', '23.00', '2019-10-30', 6960000, 12000, NULL),
(33, 1, '2019-09-09 01:21:14', 19678957, 'k', 11948618, '1', 2000000, 24, 100653, '1.50', '18.00', '19.00', '2019-10-11', 2415672, 4000, NULL),
(34, 4, '2019-09-09 01:33:44', 16355662, '6', 11948618, '1', 8573464, 24, 443704, '1.70', '20.40', '32.00', '2019-11-24', 10648896, 20000, 69054),
(35, 1, '2019-09-09 01:52:54', 19678957, 'k', 46298247, 'K', 2000000, 36, 69298, '1.20', '14.40', '20.00', '2019-10-11', 2494728, 4000, 21503),
(36, 1, '2019-09-09 01:55:05', 19678957, 'k', 46298247, 'k', 5000000, 24, 242953, '1.20', '14.40', '23.00', '2019-09-30', 5830872, 4000, 69890),
(37, 1, '2019-09-09 02:18:09', 19678957, 'k', 46298247, 'k', 2000000, 24, 99202, '1.20', '14.40', '16.50', '2019-10-11', 2380848, 4000, 16042),
(38, 1, '2019-09-09 10:37:45', 18667642, '4', 12566210, '2', 6000000, 48, 209417, '2.30', '27.60', '23.00', '2019-09-30', 10052016, 0, 86019),
(39, 1, '2019-09-09 11:48:19', 18667642, '4', 26585164, '9', 5162788, 36, 214150, '2.30', '27.60', '23.00', '2019-09-30', 7709400, 0, 72515),
(40, 1, '2019-09-09 11:48:56', 18667642, '4', 26585164, '9', 5162788, 48, 180196, '2.30', '27.60', '23.00', '2019-09-30', 8649408, 0, 72515),
(41, 1, '2019-09-09 12:18:01', 18667642, '4', 26585164, '9', 5162788, 48, 180196, '2.30', '27.60', '23.00', '2019-09-30', 8649408, 0, 72515),
(42, 1, '2019-09-09 12:20:15', 18667642, '4', 26585164, '9', 5162788, 48, 180196, '2.30', '27.60', '23.00', '2019-09-30', 8649408, 0, 72515),
(43, 1, '2019-09-09 12:34:08', 18667642, '4', 26585164, '9', 5162788, 12, 501240, '2.30', '27.60', '23.00', '2019-09-23', 6014880, 0, 72515),
(44, 1, '2019-09-09 12:58:07', 18667642, '4', 26585164, '9', 5100000, 48, 178004, '2.30', '27.60', '23.00', '2019-09-30', 8544192, 0, 71503),
(45, 1, '2019-09-09 12:59:41', 18667642, '4', 26585164, '9', 5100000, 24, 281142, '2.30', '27.60', '23.00', '2019-09-29', 6747408, 0, 71503),
(46, 1, '2019-09-09 13:52:59', 18667642, '4', 26585164, '9', 5162788, 48, 180196, '2.30', '27.60', '28.08', '2019-09-30', 8649408, 0, 72515),
(47, 1, '2019-09-09 15:00:04', 16355662, '6', 11948618, '1', 8250523, 12, 752103, '1.20', '14.40', '16.90', '2019-11-30', 9025236, 30000, 66450);
...
(7, 1, 3, 18667642, '4', 'Nicolas Ignacio', 'Poblete ', 'Peralta', 1, '2019-09-08 13:42:30', '5f4dcc3b5aa765d61d8327deb882cf99'),
(8, 1, 3, 19838453, '4', 'Valentina Javiera', 'Poblete', 'Peralta', 1, '2019-09-08 13:43:16', '5f4dcc3b5aa765d61d8327deb882cf99'),
(9, 2, 2, 11111111, '1', 'Cliente', 'Empresa', 'Scoop', 1, '2019-09-08 19:47:15', '5f4dcc3b5aa765d61d8327deb882cf99');
...
(39, 16, 4, 40641475, 19678957, '2019-09-08 13:35:47'),
(44, 29, 4, 31542065, 18667642, '2019-09-08 14:19:23'),
(42, 23, 4, 42851097, 19838453, '2019-09-08 13:55:57'),
(45, 25, 1, 11356819, 18667642, '2019-09-08 14:20:28'),
(46, 21, 4, 39025580, 18667642, '2019-09-08 14:21:07'),
(47, 31, 1, 28083441, 18667642, '2019-09-08 14:51:21'),
(48, 38, 1, 12566210, 18667642, '2019-09-09 10:39:30'),
(49, 40, 1, 26585164, 18667642, '2019-09-09 11:50:30');
...
--
ALTER TABLE `SIMULACION`
  MODIFY `IDSIMULACION` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=48;
